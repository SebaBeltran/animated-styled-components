...
import { FadeAnimations, BounceAnimations, ScaleAnimations, RotateAnimations, SlideAnimations } from './animations';
...
const Wrapper = styled.div`
  ...
  > * {
    ${({ transitionFrom }) => (transitionFrom && transitionFrom !== '' ? transitionFrom : '')};
...
class Animated extends Component {
      ...
      if (this.haveDelayIn(animation)) {
        ...
        if (this.haveAnimationContinuous(animation)) {
          ...
          if (this.haveAnimationOut(animation)) {
            ...
            await this.waitUntill(this.calculateDurationDelayBetween(animation));
  ...
  triggerDelayedTransiteOutAnimationWithInAnimation = animation => {
    return this.waitUntill(this.calculateTimeToExitWithInAnimation(animation)).then(() => {
      this.setState(function(state, props) {
        return { transite_out: true };
      });
    });
  ...
  calculateTimeToExitWithInAnimation = animation => {
    return (animation.delay_in + animation.duration_in + animation.delay_between) * 1000;
  };
  ...
  checkForValidIteration = iteration => {
    ...
    return (typeof rounds === 'number' && rounds >= 1) || iteration === 'infinite';
  };
  ...
  validateAnimation = animation => {
    ...
    if ('in' in animation) {
      ...
      if (!('duration_in' in animation)) {
        throw new TypeError('If you have an in animation you need to specify a duration for that animation');
      } else {
        // Check for Valid Animation Duration In
        if (!this.checkForValidDuration(animation.duration_in)) {
          throw new TypeError(`${animation.duration_in} is not a valid duration in for an animation`);
        }
      }
      ...
      if ('delay_between' in animation) {
        throw new TypeError(`You cannot have a delay between in and out animations if you're missing any of them`);
      }
    }
    ...
    if ('out' in animation) {
      ...
      if (!('duration_out' in animation)) {
        throw new TypeError('If you have an out animation you need to specify a duration for that animation');
      }
    } else {
      if ('delay_between' in animation) {
        throw new TypeError(`You cannot have a delay between in and out animations if you're missing any of them`);
    ...
    if ('in' in animation && 'out' in animation) {
      ...
      if (!this.checkForValidDuration(animation.duration_out)) {
        throw new TypeError(`${animation.duration_out} is not a valid duration out for an animation`);
      }
    }
    ...
    if ('continuous' in animation) {
      if ('duration_continuous' in animation) {
        ...
        if (!this.checkForValidDuration(animation.duration_continuous)) {
          throw new TypeError(`${animation.duration_continuous} is not a valid duration for a continuous animation`);
        }
      } else {
        throw new TypeError(`${animation.duration_out} is not a valid duration out for an animation`);
      }
  ...
  validateTransitions = transition => {
    if (!('type' in transition)) {
      throw new TypeError(`You're missing the type of transition property. Eg: hover, focus, blur, active, ...`);
    }
    ...
    if (!('from' in transition)) {
      throw new TypeError(`You're missing the from property of the transition that sets the point to start at`);
    }
    ...
    if (!this.checkForValidCSSProperty(transition.from.property)) {
      throw new TypeError(`${transition.from.property} is not a valid CSS property at FROM object`);
    }
    // Check for to transition object
    if (!('to' in transition)) {
      throw new TypeError(`You're missing the to property of the transition that sets the point of where to end at`);
    }
    ...
    if (!this.checkForValidCSSProperty(transition.to.property)) {
      throw new TypeError(`${transition.from.property} is not a valid CSS property at TO object`);
    }
  };
  ...
  getCurrentAnimation = () => {
    ...
    return transite_in && !transite_continuous && !transite_out
      ...
      : transite_in && transite_continuous
        ...
        : ((transite_in && !transite_continuous) || (!transite_in && !transite_continuous)) && transite_out
          ? animation.out
          : null;
  };
  ...
  getCurrentDuration = () => {
    ...
    return transite_in
      ...
      : transite_continuous ? animation.duration_continuous : transite_out ? animation.duration_out : null;
  };
  ...
  getTransitionFrom = () => {
    ...
    return transition.type === 'hover' ? `${transition.from.property}: ${transition.from.value}` : '';
  };

  getTransitionTo = () => {
    const { transition } = this.props;

    return transition.type === 'hover' ? `${transition.to.property}: ${transition.to.value};` : '';
  };

  render() {
    const { children, animation, transition } = this.props;
    const { delay_waited, transite_out, transite_in, transite_continuous } = this.state;
    return delay_waited ? (
      <Wrapper
...
export { Animated, FadeAnimations, BounceAnimations, ScaleAnimations, RotateAnimations, SlideAnimations };
